...
- July 6, 2026 (REAL-TIME PvP): live simultaneous duels shipped — `src/lib/rtc.ts` (DuelConnection over PeerJS free public broker 0.peerjs.com, WebRTC DataChannel, no server of ours) + `src/pages/RealtimeDuelPage.tsx`. Homepage "Duel a Friend — Live" → host creates room, gets `?room=CODE` link; friend opens link → joins as guest → 3-2-1 → both race same daily puzzle live. Opponent board shows colors only (letters never sent — no cheating). Winner = first to solve (elapsed-ms tiebreak); opponent disconnect mid-game awards win. Verified end-to-end over the real broker (host create, guest join, bidirectional messages, loss finalization). StrictMode-safe (DuelConnection.destroyed guard + effect cancelled flag; sends live outside setState updater to avoid double-fire). peerjs dep adds ~28KB gzip. Known: PeerJS public broker can be flaky/rate-limited — lobby shows an error + "duel the bot instead" fallback; both peers assumed same UTC day
- July 3, 2026 (async PvP): friend challenge is now a real duel — `src/lib/challenge.ts` stores your fastest win per date (`dailyduel-result-YYYY-MM-DD`); challenge link carries `?challenge=DATE&t=MS&g=N`; recipient gets "Accept Challenge — beat Xs" button + Friend Duel mode (PracticePage with `rival` prop: target banner, win = solve faster, result screen compares, XP/stats follow duel outcome); winning shares auto-append "Race me:" link with your time; homepage button renamed "Duel the Bot" (honest until real-time PvP). Real-time simultaneous PvP: planned via PeerJS public broker, not built yet
- July 3, 2026: streak/daily record now decided by FIRST completed game of the day (replays still give XP + count games, can't farm or kill streak); homepage side rails on xl+ screens — left: Rank Road ladder (thresholds mirror getRank), right: live Next Puzzle countdown (UTC midnight) + lifetime Duel Log + friend-challenge blurb; 4 extra mid-side pastel glow blobs fill hollow flanks on wide screens; rails hidden below xl, mobile unchanged
